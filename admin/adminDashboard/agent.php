...
if (isset($_GET['delete'])) {
  ...
  $delete_query = mysqli_query($con, "DELETE FROM `agent` WHERE aid=$delete_id") or die('query failed');
  if ($delete_query) {
...
<div id="confirm-modal" style="display: none;">
  ...
  <aside id="sidebar">
        ...
        <ul class="sidebar-list">
          ...
          <li class="sidebar-list-item">
           ...
           <div >
           <div class="title" style="font-size:30px;"> Agent </div>
                <table >
                    <thead>
                        ...
                        <tr>
                            <th>Agent Image</th>
                            <th> Name</th>
                            <th style="white-space: nowrap;">Agent Email</th>
                            <th>Phone</th>
                    ...
                    <tbody>
    <?php 
        $query = mysqli_query($con, "SELECT agent.*, COUNT(property.pid) AS total_properties 
                                      FROM agent 
                                      LEFT JOIN property ON agent.aid = property.aid 
                                      GROUP BY agent.aid");
        while ($row = mysqli_fetch_array($query)) {
    ?>
        <tr>
            <td><img class="w-100" src="http://localhost/rjHousing/admin/agent/<?php echo $row['aimage'];?>"> </td>
            <td><?php echo $row['aname'];?></td>
            <td style="white-space: nowrap;"><?php echo $row['aemail'];?></td>
            <td style="white-space: nowrap;"><?php echo $row['aphone'];?></td>
            <td style="white-space: nowrap;"><?php echo $row['total_properties']; ?></td>
            <td><button type="button" onclick="deleteDetails(<?php echo $row['aid']; ?>)"> Delete</button> </td>                            
        </tr>
    <?php } ?>
</tbody>
            </table>
  ...
  confirmYes.onclick = function() {
    ...
    xmlhttp.open("GET", "agent.php?delete=" + aid, true);
    xmlhttp.send();
