...
if (isset($_GET['Mdelete'])) {
  $delete_id = $_GET['Mdelete'];
  $delete_query = mysqli_query($con, "DELETE FROM `contact_admin` WHERE contact_admin_id=$delete_id") or die('query failed');
  if ($delete_query) {
    $message[]='product deleted sucessfully';
  } else {
    $message[]='product was not deleted sucessfully';
...
$query = mysqli_query($con, "SELECT agent.aname, COUNT(*) AS property_count FROM agent, property WHERE agent.aid=property.aid GROUP BY agent.aid ORDER BY property_count DESC LIMIT 10");
        $agent_names = array();
        $property_counts = array();
        while($row = mysqli_fetch_array($query)) {
            array_push($agent_names, $row['aname']);
            array_push($property_counts, $row['property_count']);
        }
...
<html lang="en">
  <head>
    ...
    <script>
      var areaChartOptions = <?php echo json_encode($areaChartOptions); ?>;
...
<div id="confirm-modal" style="display: none;">
      ...
      <main class="main-container">
        ...
        <div class="main-cards">
          <div class="card">
            <div class="card-inner">
              <p class="text-primary">PROPERTIES</p>
              <span class="material-icons-outlined text-blue"></span>
            </div>
            <span class="text-primary font-weight-bold">
            <?php
               
                $query = mysqli_query($con, "SELECT COUNT(*) FROM property ");
          ...
          <div class="card">
            <div class="card-inner">
              <p class="text-primary">ADMIN ALERTS</p>
              <span class="material-icons-outlined text-red">notification_important</span>
            </div>
            <span class="text-primary font-weight-bold">
            <?php
               
               $query = mysqli_query($con, "SELECT COUNT(*) FROM contact_admin");
               $row = mysqli_fetch_array($query);
               $total = $row[0]; 
                echo $total;  ?>  
            </span>
        ...
        <div class="charts">
          <div class="charts-card">
            <p class="chart-title">Top 10 AGENT</p>
  ...
  confirmYes.onclick = function() {
    ...
    xmlhttp.open("GET", "index.php?delete=" + pid, true);
    xmlhttp.send();
...
function deleteDetails(contact_admin_id) {
  var confirmModal = document.getElementById("confirm-modal");
  var overlay=document.getElementById("modal-overlay")
  confirmModal.style.display = "block";
  overlay.style.display="block";
  ...
  confirmYes.onclick = function() {
    ...
    xmlhttp.open("GET", "index.php?Mdelete=" + contact_admin_id, true);
    xmlhttp.send();
    confirmModal.style.display = "none";
  };
  confirmNo.onclick = function() {
    ...
    <script type="text/javascript">
      var barChartOptions = {
      series: [{
        data: <?php echo json_encode($property_counts); ?>
      }],
      chart: {
        type: 'bar',
        height: 350,
      ...
      xaxis: {
        categories: <?php echo json_encode($agent_names); ?>,
      },
      yaxis: {
        title: {
          text: "Count"
