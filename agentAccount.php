...
if (!empty($_POST['firstName'])) {
  ...
  $updates[] = "aname = '$firstname'";
}

if (!empty($_POST['email'])) {
  $email = $_POST['email'];
  $updates[] = "aemail = '$email'";
}

if (!empty($_POST['phoneNumber'])) {
  $userPhone = $_POST['phoneNumber'];
  $updates[] = "aphone = '$userPhone'";
}
...
if (!empty($_FILES['image']['name'])) {
  ...
  $image = $image_name;
  $updates[] = "aimage = '$image'";
}


if (!empty($updates)) {
    $updates_str = implode(", ", $updates);
    $sql = "UPDATE agent SET $updates_str WHERE aid = '$id'";
...
<?php include("agentHeader.php");?>
  <?php 
   ...
   $query=mysqli_query($con,"SELECT agent.* FROM `agent` WHERE aid='{$_SESSION['aid']}'");
   while($row=mysqli_fetch_array($query))
        {
       
    ?>
   <form method="post" action="agentAccount.php" id="form-update"  style="display:block; padding:3rem 10rem; margin-top:10px; border:none;"enctype="multipart/form-data" >
          <div style="text-align:center;"><p>Update Your Profile.</p></div>
             <div class="image-box" style="margin-left:11.9rem; margin-bottom:3rem; ">
                      <img style="height:200px; width:200px; border-radius:50%; "src="admin/agent/<?php echo $row['aimage']; ?>" alt="Image">
              ...
              <input type="hidden" name="aid" value="<?php echo $row['aid']; ?>">
               ...
               <div style="padding-left:8rem;">  
                ...
                <div>
                <input type="text" name="firstName"placeholder="<?php echo $row['aname'];?>"  /><br/><br/>
                </div>  
                 <label>Email</label>
                    <div >
                    <input type="text"  name="email"  placeholder="<?php echo $row['aemail'];?>">
                    </div>
                    <label >Phone Number</label>                                    
                    <div >
                    <input type="text"  name="phoneNumber"  placeholder="<?php echo $row['aphone'];?>">
                    </div>
                   </div>
